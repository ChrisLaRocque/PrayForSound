...
featuredPost = function(data){
  ...
  var template = '<div class="my-auto col-12 featured-post"><div class="col-xl-4 col-12 my-auto featured-blurb"><h2>' + featured.title + '</h2><hr><span>' + pubDate.toLocaleDateString('en-US') + '</span><p>' + featured.blurb + '</p><button class="btn btn-outline-primary w-100" data-toggle="collapse" data-target="#collapseExample' + featured.id + '" aria-expanded="false" aria-controls="collapseExample' + featured.id + '">Read Me</button></div><div class="col-11"><p class="collapse" id="collapseExample' + featured.id + '">' + featured.body + '</p></div></div>';
